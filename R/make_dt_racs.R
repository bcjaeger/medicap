...
#' @title
#' @param path_to_data
make_dt_racs <- function(path_to_data) {

  data_in <- fread(file.path(path_to_data, 'cohort_RACS.csv')) |>
    rename(
      Year = year,
      Age = age_cat,
    ...
    mutate(
      ...
      Pre_index_smoking = dplyr::recode(Pre_index_smoking,
                                        "0" = "No",
                                        "1" = "Yes"),
      Pre_index_cardiologist_visit = dplyr::recode(
  ...
  fwrite(data_in,
         file = file.path(path_to_data, 'cohort_RACS_preprocessed.csv'))
